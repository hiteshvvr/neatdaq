...
INT read_trigger_event(char *pevent, INT off)
{
  ...
  int bytes_totransfer, bytes_remaining ;
    ...
    if(bytes_remaining>1)
    {
        *ddata = (DWORD *) malloc(bytes_remaining);
        while(bytes_remaining>1)
        {
            if(bytes_remaining>4096)
                {
                    ...
                    bytes_remaining = bytes_remaining - bytes_max;
                }
            else
                {
            ...
            readmsg = CAENVME_BLTReadCycle((long *)myvme->info, V1720_BASE_ADDR+buffer_address, ddata, bytes_totransfer ,cvA32_U_BLT,0x04,& bytes_transferred);
            if(readmsg==0)
                {
                    //printf("BLT success::%d\n",readmsg);
                    ddata += bytes_transferred;
                    buffer_address += bytes_transferred/4;
                    printf("Cycles Requ:: %d \t Cycles READ:: %d \n", bytes_totransfer, bytes_transferred);
                }  
            if(readmsg!=0)
                {
                    printf("\n\n\n\nBLT REad Message::%d\n",readmsg);
                    printf("Cycles Requ:: %d \t Cycles READ:: %d \n", nuofcycles, transffered_cycles);
                }   
    }
