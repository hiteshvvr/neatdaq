/*********************************************************************
  ...
  $Id: v1720.c 5169 2011-08-02 19:44:44Z svn $
*********************************************************************/
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
uint32_t v1720_DataRead(MVME_INTERFACE *mvme, uint32_t base, uint32_t *pdata, uint32_t n32w)
{
  uint32_t i;
  uint32_t first;
  first=regRead(mvme,base,V1720_EVENT_READOUT_BUFFER);
  
  n32w=first&0x0FFFFFFF;
  n32w=n32w-1;
//  printf ("buffer::%d\n", n32w); 
  *pdata=first; pdata++;
  for (i=0;i<n32w;i++) {
...
uint32_t v1720_DataBlockRead(MVME_INTERFACE *mvme, uint32_t base, uint32_t *pdest, uint32_t *nentry)
{
  ...
  status = mvme_read(mvme, pdest, base,V1720_EVENT_READOUT_BUFFER, *nentry);
...
int  v1720_Setup(MVME_INTERFACE *mvme, uint32_t base, int mode)
{
  ...
  case 0x1:
    printf("--------------------------------------------\n");
    printf("Trigger from FP, 8channel, 1Ks, postTrigger 800\n");
    printf("--------------------------------------------\n");
    regWrite(mvme, base, V1720_BUFFER_ORGANIZATION,  0x0A);    // 1K buffer
    regWrite(mvme, base, V1720_TRIG_SRCE_EN_MASK,    0x255);  // self trigger
    regWrite(mvme, base, V1720_CHANNEL_EN_MASK,      0xFF);    // 8ch enable
    regWrite(mvme, base, V1720_POST_TRIGGER_SETTING, 500);     // PreTrigger (1K-800)
  ...
  case 0x03:
    ...
    /* /regWrite(mvme, base, V1720_CHANNEL_EN_MASK,      0xFF);    // 8ch enable */
    regWrite(mvme, base, V1720_CHANNEL_EN_MASK,      0x1F);    // 5ch enable
    /* regWrite(mvme, base, V1720_CHANNEL_EN_MASK,      0x1);    // 1ch enable */
